...
def get_validation_stats(dbconnstr):
    ...
    try:
        ...
        for x in stats_all_tmp:
            stats['stats_all'].append([x['ts'],x['num_valid'],x['num_invalid_len'],x['num_invalid_as'],x['num_not_found']])
        stats_roa_tmp = db.validity_stats.find({},{'_id':0, 'num_not_found':0})
        for y in stats_roa_tmp:
            stats['stats_roa'].append([y['ts'],y['num_valid'],y['num_invalid_len'],y['num_invalid_as']])
